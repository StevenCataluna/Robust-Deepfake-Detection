...
#         new_root = f'{root}{self.image_root}'
...
#python package_utils/geo_landmarks_extraction.py --config configs/data_preprocessing_c23.yaml --extract_landmark --save_aligned                                                                 
    ...
    def _align_face(self, image, f_lms):
        ...
        #/home/users/cataluns/LAA-Net/data/deepfake_cluster/datasets_df/FaceForensics++/c23/train/frames/Original/098/011.png
...
if __name__ == '__main__':
    ...
    # print("SAVE ALIGNED = " + str(save_aligned))
    kwargs = {}
    if extract_landmark:
    ...
    if extract_landmark:
        ...
        # print("LENGTH OF ROT IMAGES" + str(len(rot_imgs)))
        if save_aligned:
            print("SAVE ALIGNED IMAGE")
            ...
            for i, img_p in enumerate(tqdm(img_paths, dynamic_ncols=True)):
                ...
                #C:/Users/tevca/Documents/College Work/LAA-Net/data/deepfake_cluster/datasets_df/FaceForensics++/c23/train\frames\aligned_Deepfakes_81\881_856\637.png
                # aligned_img_p = img_p.replace(lm_ins.fake_types[0], f'aligned_{lm_ins.fake_types[0]}_{cfg.PREPROCESSING.N_LANDMARKS}')
                aligned_img_p = img_p.replace(lm_ins.fake_types[0], f'aligned_{lm_ins.fake_types[0]}_{cfg.PREPROCESSING.N_LANDMARKS}')
                # print(aligned_img_p)
                # print(os.path.join(lm_ins.image_root, aligned_img_p))
                # os.makedirs(os.path.join(lm_ins.image_root, aligned_img_p), exist_ok=True)
                cv2.imwrite(os.path.join(lm_ins.image_root, aligned_img_p), rot_img)
                # test_path = "C:/Users/tevca/Documents/College Work/LAA-Net/Test/" + str(i) + ".png"
                # cv2.imwrite(test_path, rot_img)
    ...
    if args.file_path is None:
        ...
        try:
            ...
            lm_ins.save2json(data, fn=f'{lm_ins.split}_{lm_ins.dataset}_FF++_{cfg.PREPROCESSING.N_LANDMARKS}.json')
        except Exception as e:
            print(e)
