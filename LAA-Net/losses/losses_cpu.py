...
# def hsic(x, y, kernel_x='gaussian', kernel_y='gaussian', sigma_x=1.0, sigma_y=1.0):
#     n_samples = x.size(0)
    
#     # Compute kernel matrices
#     if kernel_x == 'gaussian':
#         pairwise_dists_x = torch.cdist(x, x) ** 2
#         kx = torch.exp(-pairwise_dists_x / (2 * sigma_x ** 2))
#     else:  # linear kernel
#         kx = torch.mm(x, x.t())
    
#     if kernel_y == 'gaussian':
#         pairwise_dists_y = torch.cdist(y, y) ** 2
#         ky = torch.exp(-pairwise_dists_y / (2 * sigma_y ** 2))
#     else:  # linear kernel
#         ky = torch.mm(y, y.t())
    
#     # Center the kernel matrices
#     kx_centered = kx - kx.mean(dim=0, keepdim=True) - kx.mean(dim=1, keepdim=True) + kx.mean()
#     ky_centered = ky - ky.mean(dim=0, keepdim=True) - ky.mean(dim=1, keepdim=True) + ky.mean()
    
#     # Compute HSIC
#     hsic_value = (kx_centered * ky_centered).sum() / (n_samples ** 2)
#     return hsic_value
...
def f_cstency(cstency_hm_preds, cstency_hm_gt):
    ...
    cstency_matrix = torch.zeros_like(cstency_hm_gt)
...
def _neg_pos_loss(hm_pred, hm_gt):
    ...
    neg_pos_gt, neg_pos_pred = torch.zeros(batch_size, 1, dtype=torch.float64), \
...
class BaseLoss(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.hsic_lmda = self.cfg.hsic_lmda
    ...
    # def _hsic_loss(self, high, low):
    #     # assert x_high.dim() == 2 and x_low.dim() == 2, "Inputs must be 2D tensors."
    #     batch_size = high.size(0)

    #     # Center the features (subtract mean)
    #     high = high - high.mean(dim=0, keepdim=True)
    #     low = low - low.mean(dim=0, keepdim=True)

    #     # Compute Gram matrices (kernel matrices)
    #     K_high = torch.mm(high, high.t())  # (batch_size, batch_size)
    #     K_low = torch.mm(low, low.t())    # (batch_size, batch_size)

    #     # Compute HSIC
    #     hsic = torch.trace(K_high @ K_low) / (batch_size ** 2)

    #     # Normalize HSIC to make it scale-invariant
    #     hsic_normalized = hsic / (torch.norm(K_high) * torch.norm(K_low) + 1e-6)

    #     # Apply the HSIC lambda coefficient
    #     hsic_loss = hsic_normalized * self.hsic_lmda

    #     return hsic_loss
    def _hsic_loss(self, x_high, x_low):
        # Global average pooling to reduce spatial dimensions
        x_high = torch.mean(x_high, dim=(2, 3))  # Shape: (batch_size, channels)
        x_low = torch.mean(x_low, dim=(2, 3))    # Shape: (batch_size, channels)

        # Center the features
        x_high = x_high - x_high.mean(dim=0, keepdim=True)
        x_low = x_low - x_low.mean(dim=0, keepdim=True)
        batch_size = x_high.size(0)
        # Compute Gram matrices
        K_high = torch.mm(x_high, x_high.t())  # (batch_size, batch_size)
        K_low = torch.mm(x_low, x_low.t())     # (batch_size, batch_size)
...
# ORIGINAL
@LOSSES.register_module()
class CombinedHSICLoss(BaseLoss):
    def __init__(self, 
                 cfg,
                 use_target_weight, 
                 **kwargs):
        super(CombinedHSICLoss, self).__init__(cfg, **kwargs)
    ...
    def forward(self, 
        ...
        if features_high is not None and features_low is not None:
            hsic_value = self._hsic_loss(features_high, features_low)
            loss_return['hsic'] = self.hsic_lmda * hsic_value
...
#     def forward(self, 
#                 hm_outputs_high, hm_outputs_low, hm_targets, 
#                 cls_preds_high, cls_preds_low, cls_gts, 
#                 offset_preds_high, offset_preds_low=None,
#                 offset_gts=None,
#                 cstency_preds_high=None, cstency_preds_low=None,
#                 cstency_gts=None,
#                 target_weight=None,
#                 features_high=None,
#                 features_low=None):
...
#         #heatmap loss
#         hm_outputs_high_ = torch.clone(hm_outputs_high)
#         hm_outputs_low_ = torch.clone(hm_outputs_low)
#         hm_outputs_high_ = _sigmoid(hm_outputs_high_)
#         hm_outputs_low_ = _sigmoid(hm_outputs_low_)

#         if hm_targets.dim() == 3:
#             hm_targets = torch.unsqueeze(hm_targets, 1)
        
#         loss_hm_high = self.neg_loss(hm_outputs_high, hm_targets, alpha=self.hm_lmda)
#         loss_hm_low = self.neg_loss(hm_outputs_low, hm_targets, alpha=self.hm_lmda)
#         loss_return['hm'] = loss_hm_high + loss_hm_low

#         #classification loss
#         loss_cls_high = self._cls_loss(cls_preds_high, cls_gts)
#         loss_cls_low = self._cls_loss(cls_preds_low, cls_gts)
#         loss_return['cls'] = loss_cls_high + loss_cls_low
        
#         if self.dst_hm_cls_lmda > 0:
#             loss_dst_hm_cls_high = _distance_hm_cls_loss(self.cos_sim_ins, 
#                                                               hm_outputs_high, 
#                                                               hm_targets, 
#                                                               cls_preds_high, 
#                                                               cls_gts, 
...
#             loss_dst_hm_cls_low = _distance_hm_cls_loss(self.cos_sim_ins, 
#                                                               hm_outputs_low, 
#                                                               hm_targets, 
#                                                               cls_preds_low, 
#                                                               cls_gts, 
...
#             loss_return['dst_hm_cls_lmda'] = loss_dst_hm_cls_high + loss_dst_hm_cls_low

#         #Offset loss
#         if self.offset_lmda > 0 and offset_preds_high is not None:
#             loss_offset_high = self._offset_loss(offset_preds_high, 
#                                                       offset_gts, 
#                                                       apply_filter=True)
#         if self.offset_lmda > 0 and offset_preds_low is not None:
#             loss_offset_low = self._offset_loss(offset_preds_low, 
#                                                       offset_gts, 
#                                                       apply_filter=True)
#         if self.offset_lmda > 0 and offset_preds_high is not None and offset_preds_low is not None:
#             loss_return['offset'] = loss_offset_high + loss_offset_low

#         #Consistency Loss
#         if self.cstency_lmda > 0 and cstency_preds_high is not None:
#             loss_cstency_high = self._consistency_loss(cstency_preds_high, 
#                                                             cstency_gts)
#         if self.cstency_lmda > 0 and cstency_preds_low is not None:
#             loss_cstency_low = self._consistency_loss(cstency_preds_low, 
#                                                             cstency_gts)
#         if self.cstency_lmda > 0 and cstency_preds_high is not None and cstency_preds_low is not None:
#             loss_return['cstency'] = loss_cstency_high + loss_cstency_low
...
class CombinedPolyLoss(nn.Module):
    ...
    def forward(self, hm_outputs, hm_targets, cls_preds, cls_gts):
        ...
        hm_w = hm_outputs.size(3)
        total_pixels = hm_h * hm_w
        poly_loss = torch.zeros(batch_size, 1)
