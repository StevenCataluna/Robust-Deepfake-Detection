...
def HSIC(x, y, s_x=1, s_y=1):
        ...
        K = GaussianKernelMatrix(x, s_x)  # Kernel matrix for x
        L = GaussianKernelMatrix(y, s_y)  # Kernel matrix for y
        # H = torch.eye(m) - 1.0 / m * torch.ones((m, m))  # Centering matrix
        eye = torch.eye(m, dtype=x.dtype, device=x.device)
        ones = torch.ones((m, m), dtype=x.dtype, device=x.device)
        H = eye - (1.0 / m) * ones
        # H = torch.eye(m, dtype=x.dtype, device=x.device) - 1.0 / m * torch.ones((m, m), dtype=x.dtype, device=x.device)
        # H = H.double().cuda()
...
class CombinedHSICLoss(BaseLoss):
    ...
    def forward(self, 
        ...
        hm_outputs = hm_outputs.float()
        hm_targets = hm_targets.float()
        cls_preds = cls_preds.float()
        cls_gts = cls_gts.float()

        if offset_preds is not None:
            offset_preds = offset_preds.float()
        if offset_gts is not None:
            offset_gts = offset_gts.float()
        if cstency_preds is not None:
            cstency_preds = cstency_preds.float()
        if cstency_gts is not None:
            cstency_gts = cstency_gts.float()
        if features_high is not None:
            features_high = features_high.float()
        if features_low is not None:
            features_low = features_low.float()
        ...
        if features_high is not None and features_low is not None:
            features_high = features_high.view(features_high.size(0), -1)
            features_low = features_low.view(features_low.size(0), -1)
            hsic_value = HSIC(features_high, features_low)
