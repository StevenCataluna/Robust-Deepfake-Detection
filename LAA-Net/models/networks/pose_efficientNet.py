...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        if self._global_params.include_top:
            ...
            #Separate heads for high and low-quality outputs

            # self._fc_high = nn.Linear(out_channels, self._global_params.num_classes)
            #self._fc_low = nn.Linear(out_channels, self._global_params.num_classes)
    ...
    # SHARED BACKBONE
    # def forward(self, inputs_high, inputs_low):
    #     def process(inputs):
    ...
    #             return [ret], x
    #         output_high, features_high = process(inputs_high)
    #         output_low, features_low = process(inputs_low)

    #         return output_high, output_low, features_high, features_low

    #DEEPSEEK CURRENT WORKING FUNCTION
    # def forward(self, inputs_high, inputs_low):
    ...
    #     endpoints_high = self.extract_endpoints(inputs_high)
    #     endpoints_low = self.extract_endpoints(inputs_low)
    ...
    #     x1_high = endpoints_high['reduction_6']
    #     x2_high = endpoints_high['reduction_5']
    #     x3_high = endpoints_high['reduction_4']
    #     x4_high = endpoints_high['reduction_3']
    #     x5_high = endpoints_high['reduction_2']

    #     x1_low = endpoints_low['reduction_6']
    #     x2_low = endpoints_low['reduction_5']
    #     x3_low = endpoints_low['reduction_4']
    #     x4_low = endpoints_low['reduction_3']
    #     x5_low = endpoints_low['reduction_2']
    ...
    #     if self._global_params.include_top:
    #         # Pooling and final linear layer
    #         x_high = self._avg_pooling(x1_high)
    #         x_high = x_high.flatten(start_dim=1)
    #         x_high = self._dropout(x_high)
    #         x_high = self._fc(x_high)

    #         x_low = self._avg_pooling(x1_low)
    #         x_low = x_low.flatten(start_dim=1)
    #         x_low = self._dropout(x_low)
    #         x_low = self._fc(x_low)
    #         return x_high, x_low


    #     if self._global_params.include_hm_decoder:
    #         #HEATMAP DECODER FOR HIGH QUALITY
    #         x1_high = self._dropout(x1_high)
    #         x2_high = self._dropout(x2_high)
    #         x3_high = self._dropout(x3_high)
    #         x4_high = self._dropout(x4_high)

    #         if self.efpn:
    #             assert self._global_params.use_c51, "C51 must be utilized for FPN intergration"

    #             x_high = self.__getattr__('deconv_1')(x1_high)
                
    #             if self._global_params.use_c51:
    #                 x_weighted = self._sigmoid(x_high)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x2_high_ = torch.multiply(x_inv, x2_high)
    #                 x_high = torch.cat([x_high, x2_high_], dim=1)

    #                 if self.se_layer:
    #                     x_high = self.__getattr__('se_layer_1')(x_high)
    #             else:
    #                 x_high = self._relu(x_high)

    #             x_high = self.__getattr__('deconv_2')(x_high)

    #             if self._global_params.use_c4:
    #                 x_weighted = self._sigmoid(x_high)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x3_high_ = torch.multiply(x_inv, x3_high)
    #                 x_high = torch.cat([x_high, x3_high_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_high = self.__getattr__('se_layer_2')(x_high)
    #             else:
    #                 x_high = self._relu(x_high)

    #             x_high = self.__getattr__('deconv_3')(x_high)
                
    #             if self._global_params.use_c3:
    #                 assert self._global_params.use_c4, "C4 must be utilized for FPN intergration of C3"
    #                 x_weighted = self._sigmoid(x_high)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x4_high_ = torch.multiply(x_inv, x4_high)
    #                 x_high = torch.cat([x_high, x4_high_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_high = self.__getattr__('se_layer_3')(x_high)
    #             else:
    #                 x_high = self._relu(x_high)

    #             x_high = self.__getattr__('deconv_4')(x_high)
    ...
    #                 x5_high = self._dropout(x5_high)
    #                 x5_high_ = self.conv_c2(x5_high)
    #                 x_weighted = self._sigmoid(x_high)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x5_high_ = torch.multiply(x_inv, x5_high_)
    #                 x_high = torch.cat([x_high, x5_high_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_high = self.__getattr__('se_layer_4')(x_high)


    #         #HEATMAP DECODER FOR LOW QUALITY
    #         x1_low = self._dropout(x1_low)
    #         x2_low = self._dropout(x2_low)
    #         x3_low = self._dropout(x3_low)
    #         x4_low = self._dropout(x4_low)

    #         if self.efpn:
    #             assert self._global_params.use_c51, "C51 must be utilized for FPN intergration"

    #             x_low = self.__getattr__('deconv_1')(x1_low)
                
    #             if self._global_params.use_c51:
    #                 x_weighted = self._sigmoid(x_low)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x2_low_ = torch.multiply(x_inv, x2_low)
    #                 x_low = torch.cat([x_low, x2_low_], dim=1)

    #                 if self.se_layer:
    #                     x_low = self.__getattr__('se_layer_1')(x_low)
    #             else:
    #                 x_low = self._relu(x_low)

    #             x_low = self.__getattr__('deconv_2')(x_low)

    #             if self._global_params.use_c4:
    #                 x_weighted = self._sigmoid(x_low)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x3_low_ = torch.multiply(x_inv, x3_low)
    #                 x_low = torch.cat([x_low, x3_low_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_low = self.__getattr__('se_layer_2')(x_low)
    #             else:
    #                 x_low = self._relu(x_low)

    #             x_low = self.__getattr__('deconv_3')(x_low)
                
    #             if self._global_params.use_c3:
    #                 assert self._global_params.use_c4, "C4 must be utilized for FPN intergration of C3"
    #                 x_weighted = self._sigmoid(x_low)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x4_low_ = torch.multiply(x_inv, x4_low)
    #                 x_low = torch.cat([x_low, x4_low_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_low = self.__getattr__('se_layer_3')(x_low)
    #             else:
    #                 x_low = self._relu(x_low)

    #             x_low = self.__getattr__('deconv_4')(x_low)
    ...
    #                 x5_low = self._dropout(x5_low)
    #                 x5_low_ = self.conv_c2(x5_low)
    #                 x_weighted = self._sigmoid(x_low)
    #                 x_inv = torch.sub(1, x_weighted, alpha=1)
    #                 x5_low_ = torch.multiply(x_inv, x5_low_)
    #                 x_low = torch.cat([x_low, x5_low_], dim=1)
                    
    #                 if self.se_layer:
    #                     x_low = self.__getattr__('se_layer_4')(x_low)

        
    #         elif self.tfpn:
    #             assert self._global_params.use_c51, "C51 must be utilized for FPN intergration"
    #             x_high = self.__getattr__('deconv_1')(x1_high)
    #             x_high = self._relu1(x_high)
    #             x_high = torch.cat([x_high, x2_high], dim=1)
                
    #             x_high = self.__getattr__('deconv_2')(x_high)
    #             if not self._global_params.use_c4:
    #                 x_high = self._relu1(x_high)
    #             else:
    #                 x_high = torch.cat([x_high, x3_high], dim=1)
    ...
    #             else:
    #                 assert self._global_params.use_c4, "C4 must be utilized for FPN intergration of C3"
    #                 x_high = torch.cat([x_high, x4_high], dim=1)
    ...
    #                 assert self._global_params.use_c3, "C3 must be utilized for FPN intergration of C2"
    #                 x5_high = self._dropout(x5_high)
    #                 x5_high = self.conv_c2(x5_high)
    #                 x_high = self._relu1(x_high)
    #                 x_high = torch.cat([x_high, x5_high], dim=1)



    #             x_low = self.__getattr__('deconv_1')(x1_low)
    #             x_low = self._relu1(x_low)
    #             x_low = torch.cat([x_low, x2_low], dim=1)
                
    #             x_low = self.__getattr__('deconv_2')(x_low)
    #             if not self._global_params.use_c4:
    #                 x_low = self._relu1(x_low)
    #             else:
    #                 x_low = torch.cat([x_low, x3_low], dim=1)
    ...
    #             else:
    #                 assert self._global_params.use_c4, "C4 must be utilized for FPN intergration of C3"
    #                 x_low = torch.cat([x_low, x4_low], dim=1)
    ...
    #                 assert self._global_params.use_c3, "C3 must be utilized for FPN intergration of C2"
    #                 x5_low = self._dropout(x5_low)
    #                 x5_low = self.conv_c2(x5_low)
    #                 x_low = self._relu1(x_low)
    #                 x_low = torch.cat([x_low, x5_low], dim=1)
    #         else:
    #             x_low = self.deconv_layers(x1_low)
    #             x_high = self.deconv_layers(x1_high)
            
    #         # x_combined = torch.cat([x_high, x_low], dim=1)
    #         x_combined = (x_high + x_low) / 2
    #         ret = {}
    #         for head in self.heads:
    #             ret[head] = self.__getattr__(head)(x_combined)
    #         # # Final outputs for high- and low-quality features
    #         # ret_high = {}
    #         # ret_low = {}
    #         # for head in self.heads:
    #         #     ret_high[head] = self.__getattr__(head)(x_high)
    #         #     ret_low[head] = self.__getattr__(head)(x_low)
    #         # print([ret])
    #         return [ret], x1_high, x1_low
