#-*- coding: utf-8 -*-
from __future__ import absolute_import
import time

import os
import sys
if os.getcwd() not in sys.path:
    sys.path.append(os.getcwd())
import argparse
from datetime import datetime
import random

import torch
import torch.optim as optim
import torch.nn as nn
from torch.utils.data import DataLoader
from tensorboardX import SummaryWriter
import numpy as np

from configs.get_config import load_config
from models import *
from datasets import *
from losses import *
from lib.core_function import validate, train, test, train_original, validate_original
from logs.logger import Logger, LOG_DIR
from lib.optimizers.sam import SAM
from lib.scheduler.linear_decay import LinearDecayLR

from torch.amp import autocast, GradScaler


import warnings

warnings.filterwarnings('ignore')

#Path for google drive
checkpoint_dir = "/content/drive/MyDrive/LAA-Net/LAA-Net-Temp/Checkpoints"

def args_parser(args=None):
    parser = argparse.ArgumentParser("Training process...")
    parser.add_argument('--cfg', help='Config file', required=True)
    parser.add_argument('--alloc_mem', '-a',  help='Pre allocating GPU memory', action='store_true')
    return parser.parse_args(args)

# def save_checkpoint(model, optimizer, epoch, loss, scaler, path):
#     torch.save({
#         'epoch': epoch,
#         'model_state_dict': model.state_dict(),
#         'optimizer_state_dict': optimizer.state_dict(),
#         'loss': loss,
#         'scaler_state_dict' : scaler.state_dict()
#     }, path)
#     print(f"Checkpoint saved at epoch {epoch} to {path}")

def save_checkpoint(model, optimizer, epoch, loss, scaler, path):
    """Save checkpoint with guaranteed proper dtypes and devices for mixed precision training."""
    
    # 1. Prepare model state - ensure float32 for saving
    model_state = {}
    for k, v in model.state_dict().items():
        if torch.is_tensor(v):
            model_state[k] = v.float()  # Convert to float32 for saving
        else:
            model_state[k] = v
    
    # 2. Prepare optimizer state - convert all tensors to float32 except 'step'
    optimizer_state = optimizer.state_dict()
    
    # Convert parameter groups
    for group in optimizer_state['param_groups']:
        for k, v in group.items():
            if torch.is_tensor(v):
                group[k] = v.float()  # Force float32
    
    # Convert state tensors
    for state in optimizer_state['state'].values():
        for k, v in state.items():
            if torch.is_tensor(v):
                if k == 'step':
                    state[k] = v.cpu().long()  # Keep steps as CPU int64
                else:
                    state[k] = v.float()  # All other tensors to float32
    
    # 3. Prepare loss - convert if it's a tensor
    if torch.is_tensor(loss):
        loss = loss.float().item()
    elif hasattr(loss, 'avg'):  # Handle AverageMeter objects
        loss = float(loss.avg)
    
    # 4. Save everything
    torch.save({
        'epoch': epoch,
        'model_state_dict': model_state,
        'optimizer_state_dict': optimizer_state,
        'loss': loss,
        'scaler_state_dict': scaler.state_dict()
    }, path)
    
    print(f"Checkpoint saved at epoch {epoch} to {path}")

# Function to load checkpoint
def load_checkpoint(model, optimizer, scaler, path):
    checkpoint = torch.load(path, map_location='cpu', weights_only=False)
    # model.load_state_dict(checkpoint['model_state_dict'])
    # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    # if scaler is not None and 'scaler_state_dict' in checkpoint:
    #     scaler.load_state_dict(checkpoint['scaler_state_dict'])
    # epoch = checkpoint['epoch']
    # loss = checkpoint['loss']
    # print(f"Checkpoint loaded from epoch {epoch} with loss {loss}")
    # return model, optimizer, scaler, epoch, loss

    # model_state_dict = checkpoint['model_state_dict']
    # for k, v in model_state_dict.items():
    #     if torch.is_tensor(v):
    #         model_state_dict[k] = v.float()
    # model.load_state_dict(model_state_dict)
    
    # # Convert optimizer state to float32
    # optimizer_state_dict = checkpoint['optimizer_state_dict']
    # for param_group in optimizer_state_dict['param_groups']:
    #     # Handle any tensor parameters in param_groups
    #     for k, v in param_group.items():
    #         if torch.is_tensor(v):
    #             param_group[k] = v.float()
    
    # # Convert state tensors to float32
    # for state in optimizer_state_dict['state'].values():
    #     for k, v in state.items():
    #         if torch.is_tensor(v):
    #             state[k] = v.float()
    
    # optimizer.load_state_dict(optimizer_state_dict)
    
    # # Load scaler if available
    # if scaler is not None and 'scaler_state_dict' in checkpoint:
    #     scaler.load_state_dict(checkpoint['scaler_state_dict'])
    
    # # Get training metadata
    # epoch = checkpoint.get('epoch', 0)
    # # loss = checkpoint.get('loss', float('inf'))
    # loss = checkpoint['loss']
    
    # print(f"Checkpoint loaded from epoch {epoch} with loss {loss}")
    # return model, optimizer, scaler, epoch, loss

    device = 'cuda'

    model_state = {}
    for k, v in checkpoint['model_state_dict'].items():
        if torch.is_tensor(v):
            model_state[k] = v.float().to(device)  # Force float32 and target device
        else:
            model_state[k] = v
    model.load_state_dict(model_state)
    
    # 2. Optimizer state - comprehensive cleanup
    optimizer_state = checkpoint['optimizer_state_dict']
    
    # Convert parameter groups
    for group in optimizer_state['param_groups']:
        for k, v in group.items():
            if torch.is_tensor(v):
                group[k] = v.float().to(device)  # Force float32 and device
    
    # Convert state tensors with special handling for 'step'
    for state in optimizer_state['state'].values():
        for k, v in state.items():
            if torch.is_tensor(v):
                if k == 'step':  # Keep step count on CPU as long
                    state[k] = v.long().to('cpu')
                else:  # All other tensors to float32 on target device
                    state[k] = v.float().to(device)
    
    # Load into optimizer
    optimizer.load_state_dict(optimizer_state)
    
    # 3. Scaler state
    if scaler is not None and 'scaler_state_dict' in checkpoint:
        scaler.load_state_dict(checkpoint['scaler_state_dict'])
    
    # 4. Training metadata with robust printing
    epoch = checkpoint.get('epoch', 0)
    loss = checkpoint.get('loss', float('inf'))
    
    # Print loading information
    print(f"Checkpoint loaded from epoch {epoch}")
    if torch.is_tensor(loss):
        print(f"Loss: {loss.float().item():.4f}")
    elif hasattr(loss, 'avg'):
        print(f"Average loss: {float(loss.avg):.4f}")
    else:
        print(f"Loss: {float(loss):.4f}")
    
    # Clear any residual gradients
    model.zero_grad()
    optimizer.zero_grad(set_to_none=True)
    
    return model, optimizer, scaler, epoch, loss



if __name__=='__main__':
    if len(sys.argv[1:]):
        args = sys.argv[1:]
    else:
        args = None
    
    args = args_parser(args)
    cfg = load_config(args.cfg)
    logger = Logger(task=f'training_{cfg.TASK}')

    #Seed
    seed = cfg.SEED
    random.seed(seed)
    torch.manual_seed(seed)
    np.random.seed(seed)
    torch.cuda.manual_seed(seed)

    # Allocate memory
    if args.alloc_mem:
        mem_all_tensors = torch.rand(60,10000,10000)
        mem_all_tensors.to('cuda:0')

    #Configuing GPU devices
    devices = torch.device('cpu')
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True

    # print(len(cfg.TRAIN.gpus))
    # print(torch.cuda.device_count())
    print(torch.cuda.get_device_name(0))
    # if cfg.TRAIN.gpus is not None:
    #     print("GPU AVAILABLE")
    if cfg.TRAIN.gpus is not None:
        #Only support a single gpu for training now
        devices = torch.device('cuda:0')
        print('Enabled cuda')
    model = build_model(cfg.MODEL, MODELS).cuda().to(torch.float64)
    
    #Loading Dataloader
    start_loading = time.time()
    val_dataset = build_dataset(cfg.DATASET, 
                                DATASETS, 
                                default_args=dict(split='val', config=cfg.DATASET))
    val_dataloader = DataLoader(val_dataset, 
                                batch_size=cfg.TRAIN.batch_size * len(cfg.TRAIN.gpus),
                                shuffle=False,
                                pin_memory=cfg.DATASET.PIN_MEMORY,
                                num_workers=cfg.DATASET.NUM_WORKERS,
                                worker_init_fn=val_dataset.train_worker_init_fn,
                                collate_fn=val_dataset.train_collate_fn,
                                drop_last=True)
                                
    logger.info('Loading val dataloader successfully! -- {}'.format(time.time() - start_loading))
    
    start_loading = time.time()
    train_dataset = build_dataset(cfg.DATASET,
                                  DATASETS,
                                  default_args=dict(split='train', config=cfg.DATASET))
    train_dataloader = DataLoader(train_dataset, 
                                  batch_size=cfg.TRAIN.batch_size * len(cfg.TRAIN.gpus),
                                  shuffle=True,
                                  pin_memory=cfg.DATASET.PIN_MEMORY,
                                  num_workers=cfg.DATASET.NUM_WORKERS,
                                  worker_init_fn=train_dataset.train_worker_init_fn,
                                  collate_fn=train_dataset.train_collate_fn,
                                  drop_last=True)
    logger.info('Loading Train dataloader successfully! -- {}'.format(time.time() - start_loading))
    
    # start_loading = time.time()
    # test_dataset = build_dataset(cfg.DATASET, 
    #                              DATASETS,
    #                              default_args=dict(split='test', config=cfg.DATASET))
    # test_dataloader = DataLoader(test_dataset,
    #                              batch_size=cfg.TRAIN.batch_size * len(cfg.TRAIN.gpus),
    #                              shuffle=True,
    #                              pin_memory=cfg.DATASET.PIN_MEMORY,
    #                              num_workers=cfg.DATASET.NUM_WORKERS,
    #                              drop_last=True)
    # logger.info('Loading Test dataloader successfully! -- {}'.format(time.time() - start_loading))
    
    #Defining Loss function and Optimizer
    critetion = build_losses(cfg.TRAIN.loss, LOSSES, default_args=dict(cfg=cfg.TRAIN.loss)).cuda().to(torch.float64)
    if cfg.TRAIN.optimizer == 'Adam':
        optimizer = optim.Adam(model.parameters(), lr=cfg.TRAIN.lr, weight_decay=2e-5)
    elif cfg.TRAIN.optimizer == 'SAM':
        optimizer = SAM(model.parameters(), optim.Adam, lr=cfg.TRAIN.lr, weight_decay=1e-4)
    else:
        optimizer = optim.SGD(model.parameters(), lr=cfg.TRAIN.lr, weight_decay=1e-5, momentum=0.9)
    
    #Loading model
    model, optimizer, start_epoch = preset_model(cfg, model, optimizer=optimizer)
    if len(cfg.TRAIN.gpus) > 0:
        model = nn.DataParallel(model, device_ids=cfg.TRAIN.gpus).cuda().to(torch.float64)
    else:
        model = model.cuda().to(torch.float64)
    
    #Learning rate Scheduler
    if cfg.TRAIN.lr_scheduler == 'MultiStepLR':
        lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, **cfg.TRAIN.lr_scheduler)
    else:
        lr_scheduler = LinearDecayLR(optimizer, cfg.TRAIN.epochs, cfg.TRAIN.epochs//4, last_epoch=cfg.TRAIN.begin_epoch, booster=4)

    #Enabling tensorboard
    writer = SummaryWriter('.tensorboard/{}_{}'.format(datetime.today().strftime('%Y-%m-%d'), cfg.TASK))
    
    trainIters = 0
    valIters = 0
    min_val_loss = 1e10
    max_val_acc = 0
    max_test_auc = 0
    metrics_base = cfg.METRICS_BASE # Combine heatmap + cls prediction to calculate accuracy
    
    early_stopping_patience = 5  # Number of epochs to wait before stopping
    best_val_loss = float('inf')
    patience_counter = 0
    #Starting training process
    logger.info('Starting training process...')
    # print("NEW EPOCH")
    # A new version of Albumentations is available: 2.0.3 (you have 1.4.22)

    #Testing subset of dataset
    # print(f"Dataset length: {len(train_dataset)}")

    # subset = torch.utils.data.Subset(train_dataset, indices=range(500))
    # print(f"Dataset length: {len(subset)}")

    #CODE FOR RESUMING TRAINING
    # checkpoint_path = f"{checkpoint_dir}/epoch_{3}.pt"
    # model, optimizer, start_epoch, _ = load_checkpoint(model, optimizer, checkpoint_path)

    scaler = GradScaler(device='cuda')
    #CODE FOR RESUMING TRAINING
    # checkpoint_path = f"{checkpoint_dir}/epoch_{9}.pt"
    # model, optimizer, scaler, start_epoch, _ = load_checkpoint(model, optimizer, scaler, checkpoint_path)
    for epoch in range(start_epoch, cfg.TRAIN.epochs):
        #Unfreezin backbone to update weights
        if cfg.TRAIN.freeze_backbone and epoch == cfg.TRAIN.warm_up:
            unfreeze_backbone(model)
        # print("NEW EPOCH")
        np.random.seed(seed + epoch)
        if epoch > 0 and cfg.DATA_RELOAD:
            logger.info(f'Reloading data for epoch {epoch}...')
            train_dataset._reload_data()
            train_dataloader = DataLoader(train_dataset, 
                                          batch_size=cfg.TRAIN.batch_size * len(cfg.TRAIN.gpus),
                                          shuffle=True,
                                          pin_memory=cfg.DATASET.PIN_MEMORY,
                                          num_workers=cfg.DATASET.NUM_WORKERS,
                                          worker_init_fn=train_dataset.train_worker_init_fn,
                                          collate_fn=train_dataset.train_collate_fn
                                          )
        # print("FIRST TRAIN ITERATION")
        loss_avg, acc_avg, trainIters = train(cfg, 
                                              model, 
                                              critetion, 
                                              optimizer, 
                                              epoch, 
                                              train_dataloader, 
                                              logger, 
                                              writer, 
                                              devices, 
                                              trainIters,
                                              scaler, 
                                              metrics_base=metrics_base)
        if epoch % cfg.TRAIN.every_val_epochs == 0:
            loss_val, acc_val, valIters = validate(cfg, 
                                                   model, 
                                                   critetion, 
                                                   epoch, 
                                                   val_dataloader, 
                                                   logger, 
                                                   writer, 
                                                   devices, 
                                                   valIters, 
                                                   metrics_base=metrics_base)
            if not cfg.TRAIN.freeze_backbone or epoch >= cfg.TRAIN.warm_up:
                if loss_val.avg < best_val_loss:
                    best_val_loss = loss_val.avg
                    patience_counter = 0
                    ckp_path = os.path.join(LOG_DIR, '{}_{}_model_best.pth'.format(cfg.MODEL.type, cfg.TASK))
                    save_model(path=ckp_path, epoch=epoch, model=model, optimizer=optimizer)
                    logger.info(f'Saved best model at epoch --- {epoch}')
                else:
                    patience_counter += 1
                    if patience_counter >= early_stopping_patience:
                        logger.info(f'Early stopping triggered at epoch {epoch}. No improvement for {early_stopping_patience} epochs.')
                        logger.info(f'Max val accuracy = {max_val_acc}, Min val loss = {min_val_loss}')
                        print(f'EARLY STOPPING AT EPOCH {epoch}. NO IMPROVEMENT FOR {early_stopping_patience} EPOCHS')
                        break
            if acc_val.avg > max_val_acc:
                max_val_acc = acc_val.avg
            if loss_val.avg < min_val_loss:
                min_val_loss = loss_val.avg
            # if ((acc_val.avg > max_val_acc) and (loss_val.avg < min_val_loss)):
            #     # Saving checkpoint 
            #     ckp_path = os.path.join(LOG_DIR, '{}_{}_model_best.pth'.format(cfg.MODEL.type, cfg.TASK))
            #     save_model(path=ckp_path, epoch=epoch, model=model, optimizer=optimizer)
            #     min_val_loss = loss_val.avg
            #     max_val_acc = acc_val.avg
            #     logger.info(f'Saved best model at epoch --- {epoch}')

        checkpoint_path = f"{checkpoint_dir}/epoch_{epoch}.pt"
        save_checkpoint(model, optimizer, epoch, loss_avg, scaler, checkpoint_path)
        lr_scheduler.step()
