...
from datasets.CelebDFV_v2_new import CDFV2 
...
if __name__=='__main__':
    ...
    if image is not None and task == 'test_img':
        ...
        with torch.no_grad():
            st = time.time()
            img_trans = transforms(input/255).to(torch.float32)
            ...
            with autocast(device_type='cuda'):
                ...
                outputs, features = model(img_trans)
            ...
            if cfg.TEST.vis_hm:
                ...
                vis_heatmap(img, hm_preds[0], 'output_pred4.jpg')
    ...
    if task == 'eval':
        ...
        #UNCOMMENT FOR CDF                                     
        # test_dataset = CDFV2(cfg.DATASET) 
        ...
        total_preds = torch.tensor([]).cuda().to(dtype=torch.float32)
        total_labels = torch.tensor([]).cuda().to(dtype=torch.float32)
        vid_preds = {}
        vid_labels = {}
        acc = AverageMeter()
        auc = AverageMeter()
                ...
                if not video_level:
                    ...
                    for idx, vid_id in enumerate(vid_ids):
                        if vid_id in vid_preds.keys(): 
                            ...
                            vid_preds[vid_id] = torch.unsqueeze(cls_outputs[idx].clone().detach(), 0).cuda().to(dtype=torch.float32)
                            vid_labels[vid_id] = torch.unsqueeze(labels[idx].clone().detach(), 0).cuda().to(dtype=torch.float32)
                # if b == 10:
                #     break
