...
if __name__=='__main__':
    ...
    if image is not None and task == 'test_img':
        ...
        with torch.no_grad():
            ...
            if cfg.TEST.vis_hm:
                ...
                vis_heatmap(img, hm_preds[0], 'output_pred5.jpg')
    ...
    if task == 'eval':
        ...
        total_preds = torch.tensor([]).to(dtype=torch.float64)
        total_labels = torch.tensor([]).to(dtype=torch.float64)
        vid_preds = {}
        vid_labels = {}
        acc = AverageMeter()
        auc = AverageMeter()
                ...
                if not video_level:
                    ...
                    for idx, vid_id in enumerate(vid_ids):
                        if vid_id in vid_preds.keys(): 
                            ...
                            vid_preds[vid_id] = torch.unsqueeze(cls_outputs[idx].clone().detach(), 0).to(dtype=torch.float64)
                            vid_labels[vid_id] = torch.unsqueeze(labels[idx].clone().detach(), 0).to(dtype=torch.float64)
